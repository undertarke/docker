...
import { fetchFromAPI, getCommentVideoApi, getVideoIdApi } from "../utils/fetchFromAPI";
import { DOMAIN_BE_IMG, DOMAIN_BE_VIDEO } from "../utils/constants";
...
const VideoDetail = () => {
  ...
  useEffect(() => {
    ...
    getVideoIdApi(id).then(result => {
      setVideoDetail(result)
    }).catch(err => console.log(err))

    getCommentVideoApi(id).then(result => {
      setComment(result)
    }).catch(err => console.log(err))
  ...
  if (videoDetail) {
    const { video_name, user, source, views, description, likeCount } = videoDetail;

    const { user_id, full_name, avatar } = user
        ...
        <Stack direction={{ xs: "column", md: "row" }}>
          <Box flex={1}>
            <Box sx={{ width: "100%", position: "sticky", top: "86px" }}>
              ...
              {/* video main */}
              <ReactPlayer url={DOMAIN_BE_VIDEO + source} className="react-player" controls />
